...
function getTextInputClassName(size: TextInputSize, className?: string): string {
  ...
  return `w-full border border-border bg-background text-foreground outline-none placeholder:text-muted-foreground focus:bg-foreground/5 ${sizeClassName} ${className ?? ''}`.trim()
}
