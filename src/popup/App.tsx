...
function sortSiteRules(siteRules: SiteRule[]): SiteRule[] {
  return [...siteRules].sort((left, right) =>
...
export default function App() {
  ...
  const [selectedSiteRuleId, setSelectedSiteRuleId] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    const normalizedSiteRules = clearExpiredTemporaryBlocks(
      siteRules,
      currentTime,
    );
  ...
  async function addWebsite(rawInput: string): Promise<void> {
    try {
      ...
      const existingSiteRule = siteRules.find(
        (siteRule) => siteRule.domain === normalizedDomain,
      );
    ...
    try {
      await persistSiteRules(
        siteRules.map((siteRule) =>
          siteRule.id === nextSiteRule.id ? nextSiteRule : siteRule,
        ),
      );
      setErrorMessage(null);
    } catch (error) {
  ...
  async function handleRemoveSiteRule(siteRuleId: string): Promise<void> {
    try {
      await persistSiteRules(
        siteRules.filter((siteRule) => siteRule.id !== siteRuleId),
      );
        ...
        <main className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
          <div className="space-y-3">
            <form
              ...
              className="space-y-3 border border-foreground/20 p-4"
            >
              <div>
                <h2 className="text-lg font-bold">Website to block</h2>
                <p className="text-sm text-muted-foreground">
                  Add a URL or domain. YouTube hides distracting sections while
                  keeping the site accessible. Other sites show a block screen.
                </p>
              </div>
            ...
            {!isLoading && siteRules.length === 0 ? (
              <div className="border border-foreground/20 p-4">
                ...
                <p className="text-sm text-muted-foreground">
                  Add a domain to start blocking full pages or hiding specific
                  sections.
                </p>
              </div>
            ) : null}
